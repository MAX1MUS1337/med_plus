<script lang="ts">
    ...
    const ChangeData = async function() {
        const resp = await fetch("/api/dashboard/users/me/", {
            method: "POST",
            body: JSON.stringify({
                data: {
                    login: login,
    ...
    const ChangePass = async function() {
        const resp = await fetch("/api/dashboard/users/me/change_password", {
            method: "POST",
            body: JSON.stringify({
                data: {
                    old: oldPass,
    ...
    <svelte:fragment slot="header">
        <div class="absolute left-4 -bottom-6 flex items-start gap-3">
            ...
            <div class="flex flex-col justify-end self-stretch pb-12">
                <p class="font-semibold text-lg">{data.user.firstName ?? ""} {data.user.lastName ?? ""} ({data.user.login})</p>
