<script lang="ts">
    ...
    const rootUrl = "/api/offers"

    const GetOffers = async function(data?: {skip?: number, limit?: number}) {
    ...
    const CreateOffer = async function (fields: Record<string, any>) {
    ...
    const ChangeOffer = async function (key: string, value: any, fields: Record<string, any>) {
    ...
    const DeleteOffer = async function (key: string, value: any) {
        const resp = await fetch(`${rootUrl}?${key}=${value}`, {
            method: "DELETE",
        }).then(resp => resp.json())
...
<DataTable 
    ...
    fields={[
        ...
        new TableField({title: "Название", accessor: "Title", centered: true}),
        new TableField({title: "Цена", accessor: "Price", type: "number", centered: true, required: false}),
        new TableField({title: "Описание", accessor: "Description", type: "text", display: false, required: false}),
    ]}
    source={GetOffers}
    create={CreateOffer}
    change={ChangeOffer}
