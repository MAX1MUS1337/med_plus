<script lang="ts">
    ...
    const GetPatient = async function(data?: {skip?: number, limit?: number}) {
        const resp = await fetch(`${rootUrl}?skip=${data?.skip ?? ""}&limit=${data?.limit ?? ""}`).then(resp => resp.json())

        return {
            data: resp.data.patients,
            externalData: resp.data.external,
            count: resp.data.count,
        }
    }

    const CreatePatient = async function (fields: Record<string, any>) {
    ...
    const ChangePatient = async function (key: string, value: any, fields: Record<string, any>) {
    ...
    const DeletePatient = async function (key: string, value: any) {
        const resp = await fetch(`${rootUrl}?${key}=${value}`, {
            method: "DELETE",
        }).then(resp => resp.json())
...
<DataTable 
    ...
    fields={[
        ...
        new TableField({
            ...
            centered: true, 
            required: true, 
            render: (value) => ({type: "value", value: new Date(Date.parse(value)).toLocaleDateString("ru-RU")}),
        }),
        new TableField({title: "Адрес проживания", accessor: "Address", centered: true, required: true}),
